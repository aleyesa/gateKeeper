...
function gateKeeper(req, res, next) {
  ...
  const userPassReqHeader = req.get('x-username-and-password');
  const userPassReqObj = queryString.parse(userPassReqHeader);
  const findUser = USERS.find(function(userInfo) {
  
  if (userInfo.userName === userPassReqObj.user & userInfo.password === userPassReqObj.pass) {
    req.user = userInfo;
  }
  });
